...
const db = mysql.createConnection({
    host: "localhost",
    user: "root",
    STUDENT_PASSWORD: "", // Ensure this is correctly set for your environment
    database: 'school'
});
...
app.get('/studentnote/:id', (req, res) => {
    ...
    const sql = "SELECT * FROM class natural join associatedwith natural join student WHERE class.CLASS_ID = ?";
...
app.post('/classeEdit/:id', (req, res) => {
    ...
    const sql = "UPDATE class SET CLASS_NAME = ?, student_suport = ?, timining = ?, taming_coure = ? WHERE CLASS_ID = ?";
    const values = [
        req.body.CLASS_NAME,
...
app.get('/readnots/:id', (req, res) => {
    const sql = "SELECT * FROM associatedwith natural join class WHERE STUDENT_ID = ?";
    const id =req.params.id
    db.query(sql,[id], (err, result) => {
        if (err) {
            return res.json({ message: "Error inside server" });
...
app.post('/studentEdit/:id', (req, res) => {
    ...
    console.log(id);
    console.log(req.body);
    const sql = "UPDATE student SET STUDENT_NAME = ?, STUDENT_MAIL = ?, STUDENT_PASSWORD = ?, STUDENT_ADDRESS = ?, STUDENT_PHONE = ? WHERE STUDENT_ID = ?";
    const values = [
...
app.post('/student', (req, res) => {
    console.log (req.body);
    const sql = "INSERT INTO student (STUDENT_NAME, STUDENT_MAIL, STUDENT_PASSWORD, STUDENT_ADDRESS, STUDENT_PHONE) VALUES (?, ?, ?, ?, ?)";
    const values = [
        req.body.STUDENT_NAME,
        req.body.STUDENT_MAIL,
        req.body.STUDENT_PASSWORD,
...
app.post('/Admine', (req, res) => {
    ...
    console.log("--------------------");
    const sql = "INSERT INTO admine (ADMINE_NAME, ADMINE_EMAIL, ADMINE_PASSWORD, JOB, ADMINE_PHONE) VALUES (?, ?, ?, ?, ?)";
    const values = [
...
app.get('/insert_user_qcm/:id_user/:id_seggection/:qcm', (req, res) => {
    ...
    // Insert operation
    const insertSql = "INSERT INTO chose_qcm (ID_SEGGECTION, STUDENT_ID) VALUES (?, ?)";
...
app.delete('/deleteQCMuser/:id_insert/:id_user', (req, res) => {
    ...
    const sql = "DELETE FROM chose_qcm WHERE ID_SEGGECTION = ? AND STUDENT_ID = ?";
...
app.get('/classesavelebal/:id_admine', (req, res) => {
    ...
    db.query(sql, [id_admine], (err, result) => { // Pass id_admine dynamically
        if (err) {
            console.error("Error fetching data to delete:", err);
            return res.status(500).json({ error: "Internal Server Error" });
        }
...
app.get('/classesRTAvelebal/:id_admine', (req, res) => {
    ...
    db.query(sql, [id_admine], (err, result) => { // Add a comma between sql and [id_admine]
        if (err) {
            return res.json({ message: "Error inside server" }); // Standardize the key to lowercase
        }
        // console.log(result)
    ...
    try {
        ...
        db.query(query, [selectedResult, qcmTitle], (error, results) => {
            if (error) {
                ...
                return res.status(500).send({ error: 'An error occurred while creating QCM' });
            }

            // If creation is successful, send a success response
            res.status(200).send({ message: 'QCM created successfully', qcmId: results.insertId });
        });
...
app.post('/QCMcreatQestion/:submittedQuestion/:submittedNSegection/:classId/', (req, res) => {
    const { submittedQuestion, submittedNSegection, classId } = req.params;

    const query = 'INSERT INTO question (ID_QCM, QCM_QUESTION, choseNumber) VALUES (?, ?, ?)';
...
app.post('/QCMcreatSeggection/:questionId/:seggection/:note', (req, res) => {
    ...
    const query = 'INSERT INTO seggection (ID_QCM_QUESTION, SEGGECTION, SEGGECTION_TYPE) VALUES (?, ?, ?)';
...
app.post('/login/:email/:password', (req, res) => {
    ...
    const sql = "SELECT ADMIN_ID FROM admine WHERE ADMINE_EMAIL = ? AND ADMINE_PASSWORD = ? AND JOB = 'admine'";
...
app.post('/loginteacher/:email/:password', (req, res) => {
    const { email, password } = req.params;
    const sql = "SELECT ADMIN_ID FROM admine WHERE ADMINE_EMAIL = ? AND ADMINE_PASSWORD = ? AND JOB = 'teacher'";
    db.query(sql, [email, password], (err, result) => {
        if (err) {
            console.error("Error querying the database:", err);
            return res.status(500).json({ error: "Internal Server Error" });
...
app.get('/classesStudentR/:id', (req, res) => {
    const id = req.params.id;
    const sql = "SELECT * FROM class NATURAL JOIN student NATURAL JOIN associatedwith NATURAL JOIN admine WHERE class.CLASS_ID = ?";
    db.query(sql, id, (err, result) => { // Pass id as parameter to the query
        if (err) {
            return res.json({ message: "Error inside server" });
        }
        return res.json(result);
